...
export function IkigaiPremiumMap({ allActive = false, freeMode = false, landingMode = false, showPanel = true }: IkigaiPremiumMapProps) {
  ...
  const activeZone = text.zones[activeId];
  ...
  return (
    <div className={`ikigai-premium-map ${modeClass}`}>
      <svg className={landingMode ? "ikigai-svg-map" : "ikigai-svg-map has-active"} viewBox="0 0 400 430" role="img" aria-label="Ikigai Premium Map">
        <defs>
        ...
        <g opacity=".98">
          {sectors.map((sector) => (
            <g
              ...
              key={sector.id}
              onClick={locked ? undefined : () => setActive(sector.id)}
              onMouseEnter={locked ? undefined : () => setActive(sector.id)}
            >
          ...
          <g
            ...
            onClick={locked ? undefined : () => setActive("ikigai")}
            onMouseEnter={locked ? undefined : () => setActive("ikigai")}
          >
