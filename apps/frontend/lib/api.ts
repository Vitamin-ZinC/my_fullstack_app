...
export const coachApi = {
  ...
  changePlan: (id: string, idempotencyKey: string) => request<{ changed: boolean; nextChargeAt?: string | null }>(`/api/coach/subscription/change/${encodeURIComponent(id)}`, { method: "POST", headers: partnerPortalWriteHeaders(), body: JSON.stringify({ idempotencyKey }) }),
  subscriptionPortal: () => request<{ url: string }>("/api/coach/subscription/portal", { method: "POST", headers: partnerPortalWriteHeaders(), body: JSON.stringify({}) }),
